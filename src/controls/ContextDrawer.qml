...
OverlayDrawer {
    ...
    property var actions: pageStack.layers.depth > 1
        ...
        : (pageStack.visiblePages.length > 0 ? pageStack.visiblePages[pageStack.visiblePages.length - 1].contextualActions : null)
    // Disable for empty menus or when we have a global toolbar
    enabled: menu.count > 0 &&
            (typeof applicationWindow() === "undefined" || !applicationWindow().pageStack.globalToolBar || applicationWindow().pageStack.visiblePages[pageStack.visiblePages.length - 1].globalToolBarStyle !== ApplicationHeaderStyle.ToolBar)
